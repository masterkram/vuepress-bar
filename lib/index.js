...
function side(
  baseDir,
  { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, addReadMeToFirstGroup, mixDirectoriesAndFilesAlphabetically, openIndex },
  relativeDir = "",
  currentLevel = 1
) {
  ...
  if (currentLevel <= maxLevel) {
    getDirectories(join(baseDir, relativeDir))
      ...
      .forEach(subDir => {
        const children = side(
          baseDir,
          { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, openIndex },
        ...
        if (children.length > 0 || !skipEmptySidebar) {
          ...
          fileLinks.splice(insertPosition, 0, {
            ...
            initialOpenGroupIndex: openIndex,
